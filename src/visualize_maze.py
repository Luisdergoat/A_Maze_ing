...
def start_live_visualization(cell_maze, config, current_pos=None):
    ...
    if len(cell_maze) > 2 and len(cell_maze[0]) > 2:
        try:
            if hasattr(cell_maze[0][0], 'frame') and cell_maze[0][0].frame:
                has_frame = True
    ...
    panel = print_maze_visual_rich(
        maze, width, height, entry_x, entry_y, exit_x, exit_y,
        cell_maze=cell_maze, has_frame=has_frame, current_pos=current_pos
    )
    ...
    _live_display = Live(centered_panel, console=console, refresh_per_second=120)
    _live_display.start()
    return _live_display


def update_live_visualization(cell_maze, config, current_pos=None):
...
def convert_cells_to_maze_array(
    ...
    Unten=2, Links=1). Inkludiert alle Zellen inklusive Frame.
    """
...
def visualize_cell_maze(
    ...
    :param clear_screen: Wenn True, wird der Bildschirm vorher gelöscht (für Animation)
    ...
    if show_table:
        from rich.align import Align
        table = print_maze_table(maze, width, height, entry_x, entry_y, exit_x, exit_y)
...
def print_config_info(config: Dict[str, object]):
    """
...
def print_maze_visual_rich(
    ...
    # Validate dimensions
    if not maze or not maze[0]:
        return Panel("Fehler: Maze ist leer", title="Maze", border_style="red")
    ...
    return Panel(
        maze_text,
        title="[bold green]Maze Generation[/bold green]",
        border_style="green",
        box=box.ROUNDED,
        expand=False,
    )
